...
<html lang="<?= Conf::get('lang') ?>">
    <head>
        ...
        <link rel="shortcut icon" href="<?= Conf::get('page.favicon', '/favicon.ico') ?>" type="image/x-icon">
        <link rel="icon" href="<?= Conf::get('page.favicon', '/favicon.ico') ?>" type="image/x-icon">

        <?php foreach (Conf::get('page.meta', []) as $line) { ?>
            <?= $line ?>
        <?php } ?>
